        ...
        <form action="{{ route($routeName . '.destroy', $itemId) }}" method="POST" id="form-delete-{{ $itemId }}">
            @csrf
            @method('DELETE')
            <button onclick="confirmDelete(event, {{ $itemId }})" class="bg-transparent hover:bg-red-600 text-red-600 font-semibold hover:text-white py-2 px-4 border border-red-600 hover:border-transparent rounded">
